...
from led_eval.classic_cv.feature_extraction import compute_brightness_features, compute_green_features
from led_eval.classic_cv.led_state_classifier import classify_led_state
from led_eval.classic_cv.locators import FixedROILocator, SlotBasedLEDLocator, TrackingLEDLocator
from led_eval.classic_cv.preprocessing import preprocess_frame
from led_eval.classic_cv.segmentation import create_led_masks, to_value_channel
from led_eval.detectors.base_detector import BaseDetector, DetectionResult
from led_eval.utils.image_debug import draw_led_debug_overlay, save_debug_image
...
class ClassicCVDetector(BaseDetector):
    def __init__(self, config: dict[str, Any], led_layout: dict[str, dict[str, int]]) -> None:
        ...
        if locator_type == "fixed_roi":
            ...
            raise ValueError(f"Ungueltiger locator.type: {locator_type}. Erlaubt: fixed_roi, slot_based")
