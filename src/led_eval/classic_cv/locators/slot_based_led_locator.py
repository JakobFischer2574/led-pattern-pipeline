...
class SlotBasedLEDLocator(BaseLEDLocator):
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        if self.expected_led_count != 5:
            raise ValueError("SlotBasedLEDLocator erwartet expected_led_count=5 fuer dieses Routermodell.")
    ...
    def _search_bounds(self, frame_shape: tuple[int, int, int]) -> tuple[int, int, int, int]:
        ...
        if x1 <= x0 or y1 <= y0:
            raise ValueError("Ungueltige locator.search_region Konfiguration.")
        return x0, y0, x1, y1
