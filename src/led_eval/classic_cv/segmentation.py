...
def create_green_led_mask(roi_bgr: np.ndarray, config: dict[str, object]) -> tuple[np.ndarray, np.ndarray]:
    """Erstellt eine robuste Gruenmaske fuer LED-Segmentierung.

    Returns:
        (mask, exg_score)
        - mask: Binaere uint8-Maske (0/255)
...
def create_led_masks(
    ...
    """Erstellt Gruen-, Weisskern- und kombinierte LED-Maske inklusive Debug-Metriken."""
