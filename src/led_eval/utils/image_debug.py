...
def draw_detection_debug_image(
    ...
    for c in locator_data.get("original_candidates", locator_data.get("all_candidates", [])):
    ...
    for c in locator_data.get("recovered_candidates", []):
        cv2.rectangle(base, (int(c["x"]), int(c["y"])), (int(c["x"] + c["width"]), int(c["y"] + c["height"])), (0, 165, 255), 2)

    for c in locator_data.get("reconstructed_candidates", []):
        cv2.rectangle(base, (int(c["x"]), int(c["y"])), (int(c["x"] + c["width"]), int(c["y"] + c["height"])), (255, 0, 255), 2)
    ...
    draw_text(f"dynamic_roi_width: {_format_value(locator_data.get('dynamic_roi_width'))}", y0); y0 += line_h
    draw_text(f"dynamic_roi_height: {_format_value(locator_data.get('dynamic_roi_height'))}", y0); y0 += line_h
    draw_text(f"dynamic_offset_y: {_format_value(locator_data.get('dynamic_offset_y'))}", y0); y0 += line_h
    draw_text(f"geometry_score: {_format_value(locator_data.get('geometry_score'))}", y0); y0 += line_h
