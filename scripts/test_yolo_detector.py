...
from led_eval.detectors.base_detector import DetectionResult
...
def iter_frames(path: Path) -> list[Path]:
    ...
    if path.is_file():
        if path.suffix.lower() not in _IMAGE_SUFFIXES:
            raise ValueError(f"Input-Datei ist keine unterstuetzte Bilddatei: {path}")
        return [path]
...
def main() -> None:
    ...
    parser.add_argument("--input", help="Pfad zu einem einzelnen Frame oder einem Ordner mit Frames")
    parser.add_argument("--frame", help="Kompatibilitaetsalias fuer einen einzelnen Frame")
    parser.add_argument("--model", help="Ueberschreibt model_path aus der YOLO-Konfiguration")
    parser.add_argument("--config", default="configs/yolo_config.yaml", help="Pfad zur YOLO-Konfiguration")
    parser.add_argument("--debug-dir", default="data/debug/yolo", help="Ordner fuer Debug-Bilder mit Bounding Boxes")
    parser.add_argument("--output-csv", default="outputs/development_runs/yolo_results.csv", help="Pfad zur Ergebnis-CSV")
    parser.add_argument(
        "--no-debug-images",
        action="store_true",
    ...
    input_path = args.input or args.frame
    if not input_path:
        raise SystemExit("Bitte --input oder --frame angeben.")
    ...
    rows = []
    for frame_path in iter_frames(Path(input_path)):
        frame = cv2.imread(str(frame_path))
        if frame is None:
            print(f"WARN: uebersprungen (nicht lesbar): {frame_path}")
            continue
